...
class velvet():
    ...
    def __new_controls(self):
        ...
        for i in range(0, self.num_controls):
            ...
            newctl = 0
            while wegood == False:
                newctl = (np.random.randint(low=7, high=self.width), np.random.randint(low=1, high=self.height), np.random.randint(low=1, high=self.depth))
                wegood = True
    ...
    def permute(self, fraction):        
        ...
        self.layer29 = torch.divide(self.layer29, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer30 = torch.divide(self.layer30, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer31 = torch.divide(self.layer31, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer32 = torch.divide(self.layer32, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer33 = torch.divide(self.layer33, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer34 = torch.divide(self.layer34, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer35 = torch.divide(self.layer35, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer36 = torch.divide(self.layer36, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer37 = torch.divide(self.layer37, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer38 = torch.divide(self.layer38, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer39 = torch.divide(self.layer39, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer40 = torch.divide(self.layer40, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer41 = torch.divide(self.layer41, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer42 = torch.divide(self.layer42, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer43 = torch.divide(self.layer43, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer44 = torch.divide(self.layer44, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer45 = torch.divide(self.layer45, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer46 = torch.divide(self.layer46, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer47 = torch.divide(self.layer47, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer48 = torch.divide(self.layer48, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer49 = torch.divide(self.layer49, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer50 = torch.divide(self.layer50, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer52 = torch.divide(self.layer52, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer53 = torch.divide(self.layer53, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer54 = torch.divide(self.layer54, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer55 = torch.divide(self.layer55, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer56 = torch.divide(self.layer56, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer57 = torch.divide(self.layer57, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer58 = torch.divide(self.layer58, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer59 = torch.divide(self.layer59, fraction)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        self.layer60 = torch.divide(self.layer60, fraction)
