...
class velvet():
    ...
    def permute(self, fraction):        
        ...
        torch.div(temp, fraction, out=temp)
        torch.add(self.layer29, temp, out=self.layer29)
        torch.divide(self.layer29, fraction, self.layer29)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer30, fraction, self.layer30)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer31, fraction, self.layer31)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer32, fraction, self.layer32)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer33, fraction, self.layer33)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer34, fraction, self.layer34)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer35, fraction, self.layer35)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer36, fraction, self.layer36)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer37, fraction, self.layer37)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer38, fraction, self.layer38)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer39, fraction, self.layer39)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer40, fraction, self.layer40)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer41, fraction, self.layer41)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer42, fraction, self.layer42)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer43, fraction, self.layer43)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer44, fraction, self.layer44)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer45, fraction, self.layer45)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer46, fraction, self.layer46)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer47, fraction, self.layer47)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer48, fraction, self.layer48)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer49, fraction, self.layer49)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer50, fraction, self.layer50)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer52, fraction, self.layer52)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer53, fraction, self.layer53)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer54, fraction, self.layer54)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer55, fraction, self.layer55)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer56, fraction, self.layer56)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer57, fraction, self.layer57)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer58, fraction, self.layer58)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer59, fraction, self.layer59)
        temp = torch.zeros(size=(self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        random_gen = torch.Generator(device=self.device)
        random_gen.seed()
        torch.multiply(other=self.pos_propensity[0,0], input=torch.sub(other=0.5, input=torch.rand(size=(self.width, self.height, self.depth), generator=random_gen, dtype=torch.float32, device=self.device)), out=temp)
        ...
        torch.divide(self.layer60, fraction, self.layer60)
