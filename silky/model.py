...
class velvet():
    ...
    def __new_dna(self):
        ...
        self.positive_firing= torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        self.positive_resting = torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        self.negative_firing = torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        self.negative_resting = torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        self.pos_fire_amt = torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        self.neg_fire_amt = torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        self.pos_fire_amt_mult = torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
        self.neg_fire_amt_mult = torch.zeros((self.width, self.height, self.depth), dtype=torch.float32, device=self.device)
    ...
    def permute(self, degree, fraction):        
        ...
        model.__new_propensity()
        model.__new_dna()
        torch.divide(model.control_thresholds_pos, fraction, out=model.thresholds_pos)
        torch.divide(model.control_thresholds_neg, fraction, out=model.thresholds_neg)
        torch.divide(model.layer29, fraction, model.layer29)
        torch.divide(model.layer30, fraction, model.layer30)
        torch.divide(model.layer31, fraction, model.layer31)
        torch.divide(model.layer32, fraction, model.layer32)
        torch.divide(model.layer33, fraction, model.layer33)
        torch.divide(model.layer34, fraction, model.layer34)
        torch.divide(model.layer35, fraction, model.layer35)
        torch.divide(model.layer36, fraction, model.layer36)
        torch.divide(model.layer37, fraction, model.layer37)
        torch.divide(model.layer38, fraction, model.layer38)
        torch.divide(model.layer39, fraction, model.layer39)
        torch.divide(model.layer40, fraction, model.layer40)
        torch.divide(model.layer41, fraction, model.layer41)
        torch.divide(model.layer42, fraction, model.layer42)
        torch.divide(model.layer43, fraction, model.layer43)
        torch.divide(model.layer44, fraction, model.layer44)
        torch.divide(model.layer45, fraction, model.layer45)
        torch.divide(model.layer46, fraction, model.layer46)
        torch.divide(model.layer47, fraction, model.layer47)
        torch.divide(model.layer48, fraction, model.layer48)
        torch.divide(model.layer49, fraction, model.layer49)
        torch.divide(model.layer50, fraction, model.layer50)
        torch.divide(model.layer51, fraction, model.layer51)
        torch.divide(model.layer52, fraction, model.layer52)
        torch.divide(model.layer53, fraction, model.layer53)
        torch.divide(model.layer54, fraction, model.layer54)
        torch.divide(model.layer55, fraction, model.layer55)
        torch.divide(model.layer56, fraction, model.layer56)
        torch.divide(model.layer57, fraction, model.layer57)
        torch.divide(model.layer58, fraction, model.layer58)
        torch.divide(model.layer59, fraction, model.layer59)
        torch.divide(model.layer60, fraction, model.layer60)
        
        for i in range(0, degree):
            torch.add(self.control_thresholds_pos, model.thresholds_pos, out=self.thresholds_pos)
            torch.add(self.control_thresholds_neg, model.thresholds_neg, out=self.thresholds_neg)
            torch.add(self.layer29, model.layer29, out=self.layer29)
            torch.add(self.layer30, model.layer30, out=self.layer30)
            torch.add(self.layer31, model.layer31, out=self.layer31)
            torch.add(self.layer32, model.layer32, out=self.layer32)
            torch.add(self.layer33, model.layer33, out=self.layer33)
            torch.add(self.layer34, model.layer34, out=self.layer34)
            torch.add(self.layer35, model.layer35, out=self.layer35)
            torch.add(self.layer36, model.layer36, out=self.layer36)
            torch.add(self.layer37, model.layer37, out=self.layer37)
            torch.add(self.layer38, model.layer38, out=self.layer38)
            torch.add(self.layer39, model.layer39, out=self.layer39)
            torch.add(self.layer40, model.layer40, out=self.layer40)
            torch.add(self.layer41, model.layer41, out=self.layer41)
            torch.add(self.layer42, model.layer42, out=self.layer42)
            torch.add(self.layer43, model.layer43, out=self.layer43)
            torch.add(self.layer44, model.layer44, out=self.layer44)
            torch.add(self.layer45, model.layer45, out=self.layer45)
            torch.add(self.layer46, model.layer46, out=self.layer46)
            torch.add(self.layer47, model.layer47, out=self.layer47)
            torch.add(self.layer48, model.layer48, out=self.layer48)
            torch.add(self.layer49, model.layer49, out=self.layer49)
            torch.add(self.layer50, model.layer50, out=self.layer50)
            torch.add(self.layer51, model.layer51, out=self.layer51)
            torch.add(self.layer52, model.layer52, out=self.layer52)
            torch.add(self.layer53, model.layer53, out=self.layer53)
            torch.add(self.layer54, model.layer54, out=self.layer54)
            torch.add(self.layer55, model.layer55, out=self.layer55)
            torch.add(self.layer56, model.layer56, out=self.layer56)
            torch.add(self.layer57, model.layer57, out=self.layer57)
            torch.add(self.layer58, model.layer58, out=self.layer58)
            torch.add(self.layer59, model.layer59, out=self.layer59)
            torch.add(self.layer60, model.layer60, out=self.layer60)
        deg = 1 + (degree / fraction)
        torch.divide(self.control_thresholds_pos, deg + 1, out=self.thresholds_pos)
        torch.divide(self.control_thresholds_neg, deg + 1, out=self.thresholds_neg)
        torch.divide(self.layer29, deg, self.layer29)
        torch.divide(self.layer30, deg, self.layer30)
        torch.divide(self.layer31, deg, self.layer31)
        torch.divide(self.layer32, deg, self.layer32)
        torch.divide(self.layer33, deg, self.layer33)
        torch.divide(self.layer34, deg, self.layer34)
        torch.divide(self.layer35, deg, self.layer35)
        torch.divide(self.layer36, deg, self.layer36)
        torch.divide(self.layer37, deg, self.layer37)
        torch.divide(self.layer38, deg, self.layer38)
        torch.divide(self.layer39, deg, self.layer39)
        torch.divide(self.layer40, deg, self.layer40)
        torch.divide(self.layer41, deg, self.layer41)
        torch.divide(self.layer42, deg, self.layer42)
        torch.divide(self.layer43, deg, self.layer43)
        torch.divide(self.layer44, deg, self.layer44)
        torch.divide(self.layer45, deg, self.layer45)
        torch.divide(self.layer46, deg, self.layer46)
        torch.divide(self.layer47, deg, self.layer47)
        torch.divide(self.layer48, deg, self.layer48)
        torch.divide(self.layer49, deg, self.layer49)
        torch.divide(self.layer50, deg, self.layer50)
        torch.divide(self.layer51, deg, self.layer51)
        torch.divide(self.layer52, deg, self.layer52)
        torch.divide(self.layer53, deg, self.layer53)
        torch.divide(self.layer54, deg, self.layer54)
        torch.divide(self.layer55, deg, self.layer55)
        torch.divide(self.layer56, deg, self.layer56)
        torch.divide(self.layer57, deg, self.layer57)
        torch.divide(self.layer58, deg, self.layer58)
        torch.divide(self.layer59, deg, self.layer59)
        torch.divide(self.layer60, deg, self.layer60)
